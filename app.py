...
def format_product_list(products_data):
    if products_data:
        response = f"<p>Dưới đây là toàn bộ sản phẩm có trong cửa hàng của chúng tôi:</p>\n"
        for index, product in enumerate(products_data, start=1):
            if 'productname' in product:
                response += f"<p>{index}. {product['productname']}</p>\n"
    else:
        response = "<p>Xin lỗi, hiện tại không có sản phẩm nào trong cửa hàng của chúng tôi.</p>"
    return response
...
def find_discounted_products(products_data):
    ...
    if discounted_products:
        response = "<p>Dưới đây là một số sản phẩm đang được giảm giá trong cửa hàng của chúng tôi:</p>\n"
        for index, product in enumerate(discounted_products, start=1):
            if 'productname' in product:
                response += f"<p>{index}. {product['productname']}</p>\n"
    else:
        response = "<p>Xin lỗi, hiện tại chúng tôi không có thông tin về sản phẩm đang giảm giá.</p>"
