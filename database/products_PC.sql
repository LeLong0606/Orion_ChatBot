...
-- Thêm thông tin demo
INSERT INTO products (productname, price, quantity, promotion)
VALUES 
    ('Mainboard ASUS ROG Strix Z490-E Gaming', 299.99, 50, ABS(CHECKSUM(NEWID()) % 51)),
    ('Mainboard MSI MPG Z590 Gaming Carbon WiFi', 279.99, 30, ABS(CHECKSUM(NEWID()) % 51)),
    ('Mainboard Gigabyte Z590 AORUS Ultra', 319.99, 25, ABS(CHECKSUM(NEWID()) % 51)),
    ('Mainboard ASRock B550 Steel Legend', 159.99, 40, ABS(CHECKSUM(NEWID()) % 51)),
    ('Mainboard ASUS ROG Strix B550-F Gaming', 179.99, 35, ABS(CHECKSUM(NEWID()) % 51)),
    ('Mainboard MSI MAG B550 Tomahawk', 169.99, 20, ABS(CHECKSUM(NEWID()) % 51)),
    ('Mainboard Gigabyte B550 AORUS Elite', 149.99, 45, ABS(CHECKSUM(NEWID()) % 51)),
    ('Mainboard ASUS TUF Gaming B450-PLUS II', 109.99, 60, ABS(CHECKSUM(NEWID()) % 51)),
    ('Mainboard MSI B450 Tomahawk Max', 129.99, 55, ABS(CHECKSUM(NEWID()) % 51)),
    ('Mainboard Gigabyte B450M DS3H', 79.99, 70, ABS(CHECKSUM(NEWID()) % 51)),
    ('Mainboard ASRock B450M PRO4', 89.99, 65, ABS(CHECKSUM(NEWID()) % 51)),
    ('Mainboard ASUS Prime B450M-A/CSM', 99.99, 80, ABS(CHECKSUM(NEWID()) % 51)),
    ('Mainboard MSI X570 Tomahawk WiFi', 219.99, 30, ABS(CHECKSUM(NEWID()) % 51)),
    ('Mainboard Gigabyte X570 AORUS Elite', 229.99, 25, ABS(CHECKSUM(NEWID()) % 51)),
    ('Mainboard ASUS ROG Strix X570-E Gaming', 299.99, 20, ABS(CHECKSUM(NEWID()) % 51)),
    ('Mainboard ASRock X570 Phantom Gaming 4', 199.99, 35, ABS(CHECKSUM(NEWID()) % 51)),
    ('Mainboard ASUS Prime X570-P', 169.99, 40, ABS(CHECKSUM(NEWID()) % 51)),
    ('Mainboard MSI MAG X570 Tomahawk WiFi', 279.99, 15, ABS(CHECKSUM(NEWID()) % 51)),
    ('Mainboard Gigabyte B550M DS3H', 99.99, 50, ABS(CHECKSUM(NEWID()) % 51)),
    ('Mainboard ASRock B550M Steel Legend', 129.99, 45, ABS(CHECKSUM(NEWID()) % 51));
